import { isCustomerPausedOnDate, isFirstVisitOnDate } from "@/lib/customerUtils";
import type { Customer, DayJobState, DayRecord, IsoDate, OneOffJob, PaymentType } from "@/lib/models";
import { isPaymentType } from "@/lib/models";
...
function resolveJobState(state: DayJobState | undefined) {
  ...
  if (!paymentType && collected) paymentType = "cash";
  if (paymentType && !isPaymentType(paymentType)) paymentType = collected ? "cash" : "unpaid";
  return {
    cleaned,
    collected,
    visitNote: typeof state?.visitNote === "string" ? state.visitNote : "",
    smsSentAt: typeof state?.smsSentAt === "number" ? state.smsSentAt : undefined,
...
export function assembleDayJobs({
  ...
  const listed: ListedJob[] = finalOrder.map((id) => {
    ...
    const resolved = resolveJobState(state);
    const customerNotes =
      base.kind === "scheduled" && base.customer.notes ? base.customer.notes : undefined;
    const isFirstVisit =
      base.kind === "scheduled" ? isFirstVisitOnDate(base.customer, date) : false;
    return {
