...
import { earningsToCsv, sumEarnings } from "@/lib/earnings";
...
export async function GET(req: Request) {
  ...
  const result = await sumEarnings(start as IsoDate, end as IsoDate);

  if (url.searchParams.get("format") === "csv") {
    return new NextResponse(earningsToCsv(result), {
      headers: {
        "content-type": "text/csv; charset=utf-8",
        "content-disposition": `attachment; filename="earnings-${start}-${end}.csv"`,
      },
    });
  }
